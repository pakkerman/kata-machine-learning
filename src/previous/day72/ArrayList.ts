export default class ArrayList<T> {
  ...
  private data: Array<undefined | T>
  ...
  remove(item: T): T | undefined {
    for (let i = 0; i < this.length; i++) {
      if (item === this.data[i]) return this.removeAt(i)
    }
    return undefined
  }
  ...
  removeAt(idx: number): T | undefined {
    ...
    if (this.length === 0) {
      this.capacity = 3
      this.data = new Array(3).fill(undefined)
      return out
    }

    for (let i = idx; i < this.length + 1; i++) this.data[i] = this.data[i + 1]
    return out
  }
