...
export default class SinglyLinkedList<T> {
  ...
  insertAt(item: T, idx: number): void {
    ...
    if (!curr) return
    const node: Node<T> = { value: curr.value }
    this.length++
  ...
  remove(item: T): T | undefined {
    ...
    for (let i = 0; i < this.length && curr; i++) {
      if (curr.value === item) return this.removeAt(i, curr)
      curr = curr.next
    }
    return undefined
  }
  ...
  removeAt(idx: number, curr: Node<T> | undefined = undefined): T | undefined {
    if (!curr) curr = this.getNode(idx)
    console.log(curr)
